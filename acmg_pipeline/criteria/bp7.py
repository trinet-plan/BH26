...
def result(code, input_data, status, summary, **kwargs):
    ...
    message = (f"{summary.rstrip('.')}. BP7 evaluates synonymous variants outside splice-critical regions "
               f"when splice prediction evidence supports a benign interpretation. "
...
def evaluate(variant: VariantRecord, clinical_note: ClinicalNoteExtraction, services, config):
    ...
    # ClinGen SVI's 2023 splicing recommendations (Walker et al., PMID 37352859) state BP7 as
    # exactly two conditions - a synonymous/deep-intronic position outside the splice-critical
    # window, and SpliceAI predicting no impact (score <0.1) - with no separate conservation
    # requirement. A prior version of this rule additionally required `not_conserved`, which
    # made BP7 strictly harder to reach than the published criterion; that field is no longer
    # read here (see bp7.py's own git history for the correction).
    fields = ("outside_splice_critical_region", "no_predicted_splice_impact", "contradictory_rna_evidence")
    early = require_boolean_fields("BP7", input_data, evidence, assessment, fields)
    if early:
        return early
    if not all(assessment.get(key) for key in ("splice_prediction_evidence", "calibration_source", "position_rule_version")):
        return result("BP7", input_data, CriterionStatus.UNKNOWN, "Prediction/position provenance missing",
    ...
    met = all(assessment[field] for field in fields[:2])
    return result("BP7", input_data, CriterionStatus.MET if met else CriterionStatus.NOT_MET,
                  "Reviewed synonymous position and splice prediction",

...
class PopulationTests(unittest.TestCase):
    ...
    def test_excluded_small_subgroup_does_not_invalidate_usable_population(self):
        ...
        self.assertEqual(self.evaluate(pm2, self.services([usable, too_small])).status, Status.MET)
        self.assertEqual(self.evaluate(ba1, self.services([usable, too_small])).status, Status.NOT_MET)

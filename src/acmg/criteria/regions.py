...
def evaluate_pm1(input_data, region, evidence, config):
    ...
    if route not in PM1_ROUTES:
        return result("PM1", input_data, Status.NOT_EVALUATED, "Region evidence declares no PM1 route",
...
def pm1_hotspot(input_data, region, evidence, config):
    ...
    if missing_policy:
        return False, result("PM1", input_data, Status.NOT_EVALUATED, "Hotspot density policy unavailable",
                             evidence=evidence,
    ...
    if any(type(value) is not int or value < 0 for value in counts.values()):
        return False, result("PM1", input_data, Status.NOT_EVALUATED, "Hotspot variant counts unavailable",
    ...
    if conflicting:
        return False, result("PM1", input_data, Status.MANUAL_REVIEW,
